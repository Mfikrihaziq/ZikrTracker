...
export const Header: React.FC<HeaderProps> = ({ currentTab, setCurrentTab }) => {
  ...
  return (
    <header className="sticky top-0 z-30 w-full backdrop-blur-md bg-[#FAF9F6]/90 dark:bg-[#0B0F14]/90 border-b border-stone-200/80 dark:border-emerald-500/20 transition-colors dark:shadow-[0_4px_20px_-4px_rgba(16,185,129,0.08)] relative">
      ...
      {/* Auth Error Banner if popup was blocked */}
      {authError && (
        <div className="bg-amber-50 dark:bg-amber-950/60 border-t border-amber-200 dark:border-amber-800 px-4 py-2 text-xs text-amber-800 dark:text-amber-200 flex items-center justify-between">
          <span>{authError}</span>
          <button
            onClick={clearAuthError}
            className="text-xs underline font-semibold ml-2 hover:opacity-80"
          >
            Dismiss
          </button>
        </div>
